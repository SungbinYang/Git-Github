...
> 브랜치의 이름을 마음대로 지어두 될까?
물론 마음대로 지어도 뭐라고 할 사람은 없다. 하지만 협업을 할때 그 브랜치를 왜 만들었는지 기억하고 만든 목적에 맞는 이름을 지어주는 것이 좋다.
...
여기서 윗부분은 현재 브랜치의 내용이고 아래는 다른 브랜치의 내용을 비교하기 쉽게 만들어준것이다. 실제 명령어로 수정할려면 이 파일을 손을대야하지만 소스트리를 이용하면 쉽게 사용이 가능하다. 

위의 그림처럼 내것(master)로 할지 저장소(다른 브랜치)로 할지 버튼으로 결정해주면 된다. 그 후에, 커밋을 다시 해주면 된다.